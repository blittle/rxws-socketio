...
export function handleRequest(index, handlers, errorHandlers, req, res) {
	...
	if (!handlers[index]) return handleError("Resource not found", 0, errorHandlers, req, res);
	...
	if (name === 'use' || name === (req.method + '.' + req.resource)) {
		handlers[index].observer.onNext({req, res, next: (error) => {
			if (error) {
				handleError(error, 0, errorHandlers, req, res);
			} else {
...
export function handleError(error, index, errorHandlers, req, res) {
	...
	errorHandlers[index].observer.onNext({err: error, req, res, next: (newError) => {
		handleError(newError || error, index + 1, errorHandlers, req, res)
	}});
}
